...
for request_flask in webhooks:
    ...
    if (float(request_flask['hist']) > 0) and (x.get_position() is False):
        ...
        macd_15m_period.set_candles(product=request_currency, callback=get_time(83835),
        ...
        if macd_15m_period.get_hist() > 0:
            ...
            macd_5m_period.set_candles(product=request_currency, callback=get_time(27945),
            ...
            if macd_5m_period.get_hist() > 0:
                track_trade = client.place_market_order(product_id=request_currency, side="buy",
                                                        funds=get_capital('42d739b5-f5cd-48c0-baf6-b905836a1ca4',
                                                                          client=client))
    ...
    elif (float(request_flask['hist']) < 0) and (x.get_position() and (request_currency == new_order.product)):
