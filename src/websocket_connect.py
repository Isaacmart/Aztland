...
def generate_csv(a, a_file_name):
    out = re.search('"s":\[(.+?)\}\]', a).group(1)
    x = out.split(',{\"')

    with open(a_file_name, mode='w', newline='') as data_file:
        ...
        for xi in x:
            ...
            employee_writer.writerow(
                [float(xi[4]), float(xi[6]), float(xi[7]), float(xi[8]), float(xi[9])])


tickets = []
names = []
with open('txt_files/cb_tickers.txt') as data:
    for item in data:
        tickets.append(f"COINBASE:{item[:-1]}")
        names.append(item[:-1])
index = 0

for item in tickets:
    ticket = tickets[index]
    file_name = f"data/CB_stocks_{names[index]}.csv"
    args = "={\"symbol\":\"" + ticket + "\",\"adjustment\":\"splits\",\"session\":\"extended\"}"
    index += 1
    ...
    sendMessage(ws, "quote_add_symbols", [session, ticket, {"flags": ['force_permission']}])
    sendMessage(ws, "quote_fast_symbols", [session, ticket])

    # st='~m~140~m~{"m":"resolve_symbol","p":}'
    # p1, p2 = filter_raw_message(st)
    sendMessage(ws, "resolve_symbol", [chart_session, "symbol_1", args])
    sendMessage(ws, "create_series", [chart_session, "s1", "s1", "symbol_1", "1d", 5000])
